...
contract PirateHelper is PirateFeeding {
  ...
  function changeName(uint _pirateId, string calldata _newName) external aboveLevel(2, _pirateId) onlyOwnerOf(_pirateId) {
...
//delete view
  function getPiratesByOwner(address _owner) external view returns(uint[] memory) {
